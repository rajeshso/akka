...
trait RestRoutes extends BoxOfficeApi
  ...
  def eventsRoute = {
    val directive : Directive[Unit] = pathPrefix("events")
    val route : server.Route = {
      pathEndOrSingleSlash {
        get {
          // GET /events
          val onSuccessMagnet : Future[BoxOffice.Events] = getEvents()
          val onSuccessDirective : Directive[Tuple1[BoxOffice.Events]] = onSuccess(onSuccessMagnet)
          onSuccessDirective { events =>
            complete(OK, events)
          }
        }
      }
